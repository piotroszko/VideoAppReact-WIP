...
const LoginButton = () => {
  ...
  const { user } = useUser();
  return (
    <div className="relative py-3 flex justify-end items-center w-auto space-x-6">
        ...
        <Link to="/login" component={<LoginPage />}>
          <button
            className="p-3 focus:outline-none rounded text-xs font-medium leading-3
                hover:text-gray-700 hover:bg-blue-50 focus:bg-gray-700 focus:text-white text-gray-600 dark:text-gray-200 dark:border-gray-300 border-2">
            Zaloguj
          </button>
        </Link>
      </div>
