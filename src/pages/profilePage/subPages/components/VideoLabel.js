...
const VideoLabel = ({ data }) => {
  return (
    <>
      <div className="h-max hidden flex-row gap-6 justify-between mx-auto w-11/12 bg-gray-300 dark:bg-gray-700 rounded-md sm:flex">
        {data?.mediaAvaiable ? (
          <img
      ...
      {/* Mobile version */}
      <div className="h-max flex flex-col gap-6 justify-between mx-auto w-11/12 bg-gray-300 dark:bg-gray-700 rounded-md sm:hidden">
        {data?.mediaAvaiable ? (
          <img
            className={`block my-2 h-20 w-20 rounded-2xl mx-auto`}
            src={urls.thumbnails + data.id + ".png"}
            alt=""
          />
        ) : (
          <div className="flex flex-col justify-center mt-2 mx-auto my-2 w-20 h-20 text-6xl bg-gray-400 rounded-2xl">
            <div className="text-gray-300"> X </div>
          </div>
        )}
        <div className="flex flex-col gap-1 mx-auto pt-3">
          <p className="text-medium text-left dark:text-gray-200 hover:underline font-bold">
            <Link to={"/video/" + data?.id} component={<VideoPage />} className="">
              {data.name}
            </Link>
        ...
        <div className="flex flex-col gap-3 mx-auto pt-3 text-left">
          <p className="dark:text-green-500 text-green-700 text-sm font-bold">
            {data.likes} {t("likes")}
          </p>
          <p className="dark:text-red-500 text-red-700 text-sm font-bold">
        ...
        <div className="flex flex-row gap-3 items-center justify-center mb-3 mt-3 mx-auto w-full h-full sm:w-1/3">
          <Link to={"/profile/edit/" + data?.id} component={<EditVideo />} className="">
            <button className="text-bold dark:hover:bg-gray-500 my-auto p-3 h-1/2 hover:text-gray-200 dark:text-gray-200 text-lg bg-gray-200 dark:bg-gray-600 hover:bg-gray-600 rounded-lg">
              {t("edit")}
            </button>
          </Link>
          <button className="text-bold dark:hover:bg-red-700 my-auto p-3 h-1/2 hover:text-gray-200 dark:text-gray-200 text-lg bg-gray-200 hover:bg-gray-600 dark:bg-red-800 rounded-lg">
            {t("delete")}
          </button>
        </div>
      </div>
